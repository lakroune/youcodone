        ...
        <div class="w-full max-w-5xl flex flex-col md:flex-row gap-12 items-center md:items-start">
            ...
            <div class="md:w-1/3 flex flex-col items-center">
                <div class="relative group">
                    <div class="w-48 h-48 rounded-full overflow-hidden avatar-ring transition-transform duration-500 group-hover:rotate-3">
                    ...
                    <form action="{{ ('profile.avatar.update') }}" method="POST" enctype="multipart/form-data" id="avatarForm">
                        @csrf
                        <label class="absolute bottom-4 right-4 bg-[#FF5F00] p-3 rounded-full cursor-pointer hover:scale-110 transition-all shadow-xl shadow-black">
                ...
                <div class="mt-8 text-center">
                    <h2 class="text-3xl font-black italic uppercase tracking-tighter text-glow">{{ Auth::user()->username }}</h2>
                    <div class="mt-2">
                        <span class="px-4 py-1 bg-[#FF5F00]/10 text-[#FF5F00] text-[10px] font-black uppercase tracking-[2px] rounded-full border border-[#FF5F00]/20">
            ...
            <div class="md:w-2/3 profile-card rounded-3xl p-10 shadow-2xl relative overflow-hidden">
                ...
                <form action="{{ ('profile.update') }}" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" name="phone" value="{{ old('phone', Auth::user()->phone) }}" placeholder="+212 ..."
                            class="w-full bg-[#1A1A1A] border border-white/10 rounded-xl px-5 py-4 text-sm text-white outline-none orange-glow transition-all focus:bg-black">
                        <x-input-error class="mt-2" :messages="$errors->get('phone')" />
                    </div>

                    <div class="space-y-2 md:col-span-2">
                        <label class="text-[10px] text-gray-500 font-black uppercase tracking-[3px]">Shipping Address</label>
                        <input type="text" name="address" value="{{ old('address', Auth::user()->address) }}" placeholder="Rue, Ville, Code Postal"
                            class="w-full bg-[#1A1A1A] border border-white/10 rounded-xl px-5 py-4 text-sm text-white outline-none orange-glow transition-all focus:bg-black">
                    ...
                    <div class="md:col-span-2 flex items-center gap-3 py-4">
                        ...
                        <span class="text-[10px] text-gray-500 uppercase font-black tracking-[2px]">Account Status: <span class="text-white">{{ Auth::user()->status ?? 'Active' }}</span></span>
                    </div>

                    <div class="md:col-span-2 pt-6 flex flex-col sm:flex-row gap-4">
                        <button type="submit" class="bg-[#FF5F00] hover:bg-[#E65600] text-white text-[11px] font-black px-10 py-5 rounded-xl uppercase tracking-[3px] transition-all transform active:scale-95 shadow-2xl shadow-[#FF5F00]/20">
                            Save Changes
                        </button>
                        
                        <a href="{{ route('dashboard') }}" class="inline-flex justify-center items-center bg-white/5 hover:bg-white/10 text-white text-[11px] font-black px-10 py-5 rounded-xl uppercase tracking-[3px] transition-all border border-white/5">
                            Cancel
                        </a>

                        @if (session('status') === 'profile-updated')
                            <p x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 3000)" class="text-sm text-[#FF5F00] font-bold self-center animate-pulse">
