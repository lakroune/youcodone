<x-app-layout>
            ...
            <div x-show="step === 1" x-transition.opacity.duration.400ms class="space-y-6">
                <div class="bg-[#0A0A0A] border border-white/5 p-10 rounded-[2.5rem]">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        {{-- email --}}
                        <div class="space-y-2">
                            <label class="text-[10px] text-gray-500 font-black uppercase tracking-[2px]">Email</label>
                        ...
                        <div class="space-y-2">
                            <label
                                class="text-[10px] text-gray-500 font-black uppercase tracking-[2px]">Téléphone</label>
                            <input type="text" name="telephone_restaurant"
                                class="w-full bg-[#111] border border-white/5 rounded-2xl px-5 py-4 text-white focus:border-[#FF5F00] transition-all outline-none">
                        </div>
                        {{-- capacity --}}
                        <div class="space-y-2">
                        ...
                        <div class="col-span-1 md:col-span-2 space-y-2">
                            <label
                                class="text-[10px] text-gray-500 font-black uppercase tracking-[2px]">Description</label>
                            <textarea name="description_restaurant" rows="4"
                                class="w-full bg-[#111] border border-white/5 rounded-2xl px-5 py-4 text-white focus:border-[#FF5F00] transition-all outline-none resize-none"></textarea>
                        </div>
                    </div>
                </div>
            ...
            <div x-show="step === 3" x-transition.opacity.duration.400ms class="space-y-6">
                ...
                <div class="bg-[#0A0A0A] border border-white/5 p-10 rounded-[2.5rem]">
                    ...
                    <div id="menu-container" class="space-y-3">
                        <div class="flex gap-4">
                            <input type="text" name="menu[0][nom_plat]" placeholder="Plat"
                                class="flex-1 bg-[#111] border border-white/5 rounded-xl px-5 py-4 text-xs text-white">
                            <input type="number" name="menu[0][prix_plat]" placeholder="DH"
    ...
    <script>
        ...
        function addMenuRow() {
            ...
            div.className = 'flex gap-4 animate-fade-in mt-3';
            div.innerHTML = `
        <input type="text" 
               name="menu[${menuIdx}][nom_plat]" 
               placeholder="Plat" 
               class="flex-1 bg-[#111] border border-white/5 rounded-xl px-5 py-4 text-xs text-white outline-none focus:border-[#FF5F00] transition-all">
        
        <input type="number" 
               name="menu[${menuIdx}][prix_plat]" 
               placeholder="DH" 
               class="w-24 bg-[#111] border border-white/5 rounded-xl px-5 py-4 text-xs text-white outline-none focus:border-[#FF5F00] transition-all">
        
        <button type="button" 
                onclick="this.parentElement.remove()" 
                class="text-red-500 hover:text-red-400 font-bold px-2 transition-colors">
            ×
        </button>
    `;
